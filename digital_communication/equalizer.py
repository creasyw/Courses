...
def binary_pam(nsample):
    ...
    return np.array(map(lambda x: -1 if x == 0 else x,
                        np.random.randint(2,
                                          size=nsample)))
    ...
    for l in range(nzf):
        ...
        for j in range(nzf):
            for n in range(len(fz)):
                if 0 < n + l - j < len(fz):
                    r[l, j] += np.conjugate(fz[n]) * fz[n + l - j]
    ...
    for l in range(k1):
        ...
        for j in range(k1):
            for n in range(len(tap)):
                if 0 < n + l - j < len(tap):
                    r[l, j] += np.conjugate(tap[n]) * tap[n + l - j]
    ...
    for unknown in product(values, repeat=len(taps)):
        ...
        for i in range(L + k - 1):
            ...
            if len(current_in_loop) < L:
                current_in_loop = np.array([0] * (L - len(current_in_loop)) +
                                           current_in_loop)
            else:
...
def equalizer(nchannel, nzf, snrlst, nsample, eqtype):
    ...
    for i in range(len(snrlst)):
        ...
        if eqtype == "zfir":
            ...
            raise ValueError(
                "The type of equalizer should be 'zf' (zero-forcing) or\
                    lmmse (Linear Minimumu MSE)!")

        if eqtype != "dfe" and eqtype != "viterbi":
            estimate = np.array(map(lambda x: -1 if x < 0 else 1,
                                    np.convolve(vn,
                                                coeff,
                                                mode='same')))
        elif eqtype == "dfe":
        ...
        ser[i] = sum(1 for i in range(nsample)
                     if samples[i] != estimate[i]) / float(nsample)
    return ser
