...
%     C(m)=sum(log2(1+Pl(m)*(h11.^2+h21.^2)/N0/2))/L; % channel is not known by transmitter
    C(m)=sum(log2(1+Pl(m)*(h11.^2+h21.^2)/N0))/L; % channel is known by transmitter
...
title('channel capacity of CDMA2000 with equal power allocation (MISO2) CSI');
% title('channel capacity of CDMA2000 with equal power allocation (MISO2) CSIR');