...
#+begin_quote
Until now, there are a few keywords:

- =data= links /type constructor/ to the /data constructor/.
  - There are two kinds of types.
  - =Maybe= is an /abstrct type/.
  - Something like =TrafficLight= is a /concrete type/.
- =type= provides a /type synonyms/.
- =class= constructs a =typeclass=, which provides general /interface/ (or contracts) of the classes that derive from this typeclass.
  ...
  - =instance= is the keyword to make a *concrete type* comply with the "contract" of a typeclass
#+end_quote
