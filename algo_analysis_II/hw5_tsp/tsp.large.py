    ...
    while True:
        ...
        for s in combinations(range(len(graph)), m):
            ...
            for j in s:
                ...
                current_dict[cur_name][j] = min(
                    old[old_name][k] + dict[k, j]
                    for k in temp if k != j and k in old[old_name])
        m += 1
        if m >= len(graph) / 2 + 2: break
        old = {}
        for i in current_dict:
            old[i] = {}
            temp = np.array(sorted(current_dict[i].iteritems(),
                                   key=operator.itemgetter(1))[:4])
            for j in temp[:, 0]:
