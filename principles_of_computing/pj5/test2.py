...
def Zombie_Appocalypse_test():
    ...
    zombies = [(0, 0)]
    suite.run_test(zombie._zombie_list, zombies,
                   "Test 2.0 : Zombie.add_zombie")
    ...
    for zom in zombie.zombies():
        zomb += str(zom)
    suite.run_test(zomb, "(0, 0)(0, 1)",
                   "Test 4.0, Zombie class, zombies method")

    # Test 5.0 : Zombie class, add_human method
    zombie.add_human(0, 2)
    humans = [(0, 2)]
    suite.run_test(zombie._human_list, humans,
                   "Test 5.0, Zombie class, add_human method")

    # Test 6.0 Zombie class, num_human method
    suite.run_test(zombie.num_humans(), 1,
                   "Test 6.0, Zombie class, num_human method")
    ...
    row2 = [3, 2, 1, 2, 3]
    suite.run_test(
        a.compute_distance_field(ZOMBIE)[1], row2,
        "Test 8.0, Zombie class, compute_distance_field")
    ...
    suite.run_test(
        a.compute_distance_field(ZOMBIE)[1], row2,
        "Test 8.1, Zombie class, compute_distance_field")
