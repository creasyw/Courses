...
class TreeDisplay:
    ...
    def draw_tree(self, canvas, tree, pos):
        ...
        for idx in range(len(horiz_boundaries) - 1):
            child_center = [(horiz_boundaries[idx] + horiz_boundaries[idx + 1])
                            / 2, pos[1] + 3 * NODE_HEIGHT / 2]
            canvas.draw_line(root_center, child_center, 3, "Black")

        # draw root
        canvas.draw_circle(root_center, NODE_HEIGHT / 4, 2, "Black",
                           "LightGreen")
        text_pos = [root_center[0] - NODE_HEIGHT / 12,
                    root_center[1] + NODE_HEIGHT / 12]
